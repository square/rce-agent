...
func NewServer(port string, configFile string) (RCEAgentServer, error) {
...
// server.
func (s *rceAgentServer) Start() error {
...
func (s *rceAgentServer) Stop() error {
	s.grpcServer.GracefulStop()
	return nil
}

// GetJobs list all jobs that have started after since.
func (s *rceAgentServer) GetJobs(since *pb.StartTime, stream pb.RCEAgent_GetJobsServer) error {
...
func (s *rceAgentServer) GetJobStatus(ctx context.Context, id *pb.JobID) (*pb.JobStatus, error) {
	fmt.Printf("Status request for %d\n", id.JobID)
	return s.getJobStatus(id.JobID)
}

// GetJobStatus returns the job status of the given job id.
func (s *rceAgentServer) getJobStatus(id uint64) (*pb.JobStatus, error) {
	s.jobsM.RLock()
	job, ok := s.allJobs[id]
	s.jobsM.RUnlock()
	if !ok {
...
func (s *rceAgentServer) StopJob(ctx context.Context, id *pb.JobID) (*pb.JobStatus, error) {
	...
	if job.Status.Status != int64(pb.Running) {
...
func (s *rceAgentServer) StartJob(ctx context.Context, details *pb.JobRequest) (*pb.JobStatus, error) {
...
func (s *rceAgentServer) runJob(job *runningJob) {
	...
	if !ok {
		...
		job.statusM.Lock()
		job.Status.Status = int64(pb.Completed)
	...
	job.Status.Status = int64(pb.Running)
...
func (s *rceAgentServer) getNewJobID() uint64 {
	s.IDm.Lock()
	defer s.IDm.Unlock()
	myID := s.nextID
	s.nextID++
